...
if (!inputFile) {
  console.error('Usage: node generate-v6.js <model.json>')
  process.exit(1)
}
...
// --- derive entity name from schema_name ---
// employees -> Employee, employee_records -> EmployeeRecords
...
export default function ${entity}Form({ initialData = null, cancelHref = '/${base_route}' }) {
...
export default async function Edit${entity}Page({ params }) {
  ...
  return (
    <div className="p-6 flex justify-center">
      <${entity}Form initialData={record} cancelHref="/${base_route}" />
    </div>
  )
}
`
